...
class IotIncharge extends StatefulWidget {
  const IotIncharge({super.key});

  @override
  State<IotIncharge> createState() => _IotInchargeState();
}

class _IotInchargeState extends State<IotIncharge> {
                ...
                ElevatedButton(
                    onPressed: () async {
                      ...
                      if (name.isNotEmpty && count > 0) {
                        await FirebaseFirestore.instance.collection('Iotequipment').add(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('IOT Lab Equipment'),
        actions: [
          IconButton(
          ...
          children: [
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance.collection('Iotequipment').snapshots(),
